...
const sonartsServer_1 = require("../runner/sonartsServer");
/**
...
function parseString(source, scriptKind = ts.ScriptKind.TSX) {
    ...
    const host = Object.assign({}, ts.createCompilerHost({ strict: true }), { getSourceFile: () => ts.createSourceFile(filename, source, TARGET, true, scriptKind), getCanonicalFileName: () => filename });
    const program = ts.createProgram([], { strict: true }, host);
    return { sourceFile: program.getSourceFiles()[0], program };
}
exports.parseString = parseString;
function parseStringAsSourceFile(source, scriptKind = ts.ScriptKind.TSX) {
    return ts.createSourceFile("filename.ts", source, TARGET, true, scriptKind);
}
exports.parseStringAsSourceFile = parseStringAsSourceFile;
/**
...
function createProgram(configFile, projectRoot) {
    const { options, files } = parseTsConfig(configFile, projectRoot);
    const host = ts.createCompilerHost(options, true);
    return ts.createProgram(files, options, host);
}
exports.createProgram = createProgram;
function parseTsConfig(tsConfig, projectRoot) {
    let projectDirectory = projectRoot;
    let config = { config: { compilerOptions: {} } };
    if (tsConfig !== sonartsServer_1.DEFAULT_TSCONFIG) {
